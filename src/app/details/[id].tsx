...
export default function Details() {
  ...
  function fetchDetails() {
    try {
      if (goalId) {
        ...
        const transactions = useTransaction.show(goalId);
  ...
  async function handleDeleteTransaction(idTransaction: number) {
    try {
      Alert.alert("Excluir", "Deseja excluir essa transação?", [
        {
          text: "Sim", onPress: () => [useTransaction.deleteTransaction(idTransaction), fetchDetails()]
        },
  ...
  async function handleNewTransaction() {
    try {
      ...
      useTransaction.create({ goal_id: goalId, amount: amountAsNumber })
  ...
  function handleGoalDelete(goalId: number) {
    try {
      Alert.alert("Excluir", "Deseja excluir esse Objetivo?", [
        {
          text: "Sim", onPress: () => [useGoal.DeleteGoal(goalId), router.back()]
        },
        { text: "Não", style: "cancel" }
      ])
    }
...
return (
  <View className="flex-1 p-8 pt-12">
    ...
    <Transactions transactions={goal.transactions} onDeleteTransaction={handleDeleteTransaction} />

    <Button color="bg-blue-500" title="Nova transação" onPress={handleBottomSheetOpen} />
    <Button color="bg-red-600" title="Excluir Objetivo" onPress={() => handleGoalDelete(goalId)} />
    ...
    >
      ...
      <Button color="bg-blue-500" title="Confirmar" onPress={handleNewTransaction} />
    </BottomSheet>
  </View>
)
}
